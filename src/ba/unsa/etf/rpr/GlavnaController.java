...
public class GlavnaController {
    ...
    public void actionBrisanjeDrzave(ActionEvent actionEvent) {
        Stage stage = new Stage();
        Parent root = null;
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/fxml/brisanje.fxml"));
            BrisanjeController brisanjeController = new BrisanjeController(dao.drzave());
            loader.setController(brisanjeController);
            root = loader.load();
            stage.setTitle("Brisanje države");
            stage.setScene(new Scene(root, USE_COMPUTED_SIZE, USE_COMPUTED_SIZE));
            stage.setResizable(true);
            stage.show();

            stage.setOnHiding( event -> {
                if (brisanjeController.odabrana != null) {
                    // Ovdje ne smije doći do izuzetka jer se prozor neće zatvoriti
                    try {
                        dao.obrisiDrzavu(brisanjeController.odabrana, brisanjeController.saGradovima);
                        listGradovi.setAll(dao.gradovi());
                    } catch (NemoguceBrisanjeException e) {
                        Alert alert = new Alert(Alert.AlertType.ERROR);
                        alert.setTitle("Nemoguće obrisati državu");
                        alert.setHeaderText("Nije uspjelo brisanje države " + brisanjeController.odabrana.getNaziv());
