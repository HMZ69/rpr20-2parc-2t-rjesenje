...
public class GeografijaDAO {
    ...
    public void obrisiDrzavu(Drzava odabrana, boolean saGradovima) {
        if (!saGradovima) {
            String poruka = "Nije moguće obrisati državu jer sadrži gradove: ";
            int count=0;
            for(Grad grad: gradovi())
                if (grad.getDrzava().getId() == odabrana.getId()) {
                    poruka += grad.getNaziv() + ", ";
                    count++;
                }
            if (count > 0)
                throw new NemoguceBrisanjeException(poruka);
        }
        obrisiDrzavu(odabrana.getNaziv());
    }
}
